...
import { FaPlane, FaShieldAlt, FaHeadset, FaCheckCircle, FaPercent, FaArrowRight } from "react-icons/fa";
import { MdLocalOffer } from "react-icons/md";

// Lazy load heavy component
const CountryPage = dynamic(() => import("./components/CountryPage"), {
  loading: () => (
...
const POPULAR_DESTINATIONS = [
  { name: "Dubai", image: "https://images.unsplash.com/photo-1512453979798-5ea266f8880c?w=400&q=75", flights: "500+ flights" },
  { name: "Singapore", image: "https://images.unsplash.com/photo-1525625293386-3f8f99389edd?w=400&q=75", flights: "320+ flights" },
  { name: "London", image: "https://images.unsplash.com/photo-1513635269975-59663e0ac1ad?w=400&q=75", flights: "450+ flights" },
  { name: "New York", image: "https://images.unsplash.com/photo-1496442226666-8d4d0e62e6e9?w=400&q=75", flights: "380+ flights" },
  { name: "Paris", image: "https://images.unsplash.com/photo-1502602898657-3e91760cbb34?w=400&q=75", flights: "290+ flights" },
  { name: "Tokyo", image: "https://images.unsplash.com/photo-1540959733332-eab4deabeeaf?w=400&q=75", flights: "210+ flights" }
];
...
const Home = () => {
  ...
  useEffect(() => {
    ...
    const updateCountryName = () => {
      setCountryName(sessionStorage.getItem("country_name") || "WorldWide");
    };
  ...
  useEffect(() => {
    fetchData(sessionStorage.getItem("country_name") || "WorldWide");
  }, [countryName, fetchData]);

  const heroData = useMemo(() => apiData?.[0]?.hero || null, [apiData]);

  // Loading state - show search bar immediately
  if (loading) {
    return (
  ...
  return (
    <div className="flex flex-col min-h-screen bg-[#f4f4f4]">
      ...
      <section className="container mx-auto px-4 py-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {OFFERS.map((offer, index) => (
            ...
            >
              <span className="text-xs font-semibold bg-white/20 px-2 py-1 rounded">{offer.title}</span>
              <h3 className="text-2xl font-bold mt-3 mb-1">{offer.discount}</h3>
              <p className="text-white/80 text-sm mb-3">{offer.desc}</p>
              <div className="flex items-center justify-between">
                <span className="text-xs bg-white/20 px-2 py-1 rounded font-mono">{offer.code}</span>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {TRUST_BADGES.map((badge, index) => (
            <div key={index} className="trust-badge">
              <div className={`icon ${badge.color}`}>{badge.icon}</div>
              <div>
      ...
      {/* Popular Destinations with Optimized Images */}
      <section className="container mx-auto px-4 py-12">
        <div className="flex items-center justify-between mb-6">
          <div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          {POPULAR_DESTINATIONS.map((dest, index) => (
            <div key={index} className="destination-card card-elevated relative overflow-hidden">
              <Image 
                src={dest.image} 
                alt={dest.name}
                width={200}
      ...
      <section className="bg-white py-16">
        <div className="container mx-auto px-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {[
              { icon: <FaPlane className="text-3xl text-[#008cff]" />, title: "500+ Aircraft", desc: "Access to the largest fleet of private jets, helicopters and air ambulances worldwide" },
              { icon: <FaHeadset className="text-3xl text-[#ff6b00]" />, title: "24/7 Support", desc: "Round-the-clock customer service to assist you at every step of your journey" },
              { icon: <FaShieldAlt className="text-3xl text-[#4caf50]" />, title: "Safety First", desc: "All aircraft are fully certified and meet the highest safety standards" },
              { icon: <FaPercent className="text-3xl text-[#9c27b0]" />, title: "Best Prices", desc: "Competitive pricing with no hidden charges. Price match guarantee available" }
            ].map((feature, index) => (
              <div key={index} className="text-center p-6 card-elevated">
                <div className="icon-circle mx-auto mb-4" style={{ background: '#f5f5f5' }}>{feature.icon}</div>
      ...
      {/* Country Page Content - Lazy Loaded */}
      <CountryPage apiData={apiData} />

      {/* WhatsApp Float */}
      <a href="https://wa.me/919999999999" target="_blank" rel="noopener noreferrer" className="whatsapp-float" aria-label="WhatsApp">
        💬
      </a>
    </div>
  );
